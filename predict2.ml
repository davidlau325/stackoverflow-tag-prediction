	...
	let (tagName,baseLevel) = get_data ("tags/"^tag) 0 3 in
		let activate = (Float.of_string baseLevel) +.
			...
			~f:(fun acc word ->
				...
				if Sys.file_exists_exn word_path then begin
				let (_,eJ) = get_data ("words/"^word^"/_count.txt") 0 3 in
				let (_,sAssoc) = get_data word_path 0 3 in
...
let print_activate activate_list =
	...
	~f:(fun fd ->
		match activate_list with
		| x1::x2::x3::x4::x5::xs -> begin 
							Out_channel.output_string fd (x1.tag_name ^ " " ^ (Float.to_string x1.act) ^ "\n")
							Out_channel.output_string fd (x2.tag_name ^ " " ^ (Float.to_string x2.act) ^ "\n")
							Out_channel.output_string fd (x3.tag_name ^ " " ^ (Float.to_string x3.act) ^ "\n")
							Out_channel.output_string fd (x4.tag_name ^ " " ^ (Float.to_string x4.act) ^ "\n")
							Out_channel.output_string fd (x5.tag_name ^ " " ^ (Float.to_string x5.act) ^ "\n")
								end
		| _ -> ()
	);
	incr countTest
